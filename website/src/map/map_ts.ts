...
class Map {
    ...
    private projection!: (coordinates: [number, number]) => [number, number]|null;

    private heatmapMode!: string | null;

    private trainObjects!: { [key: string]: THREE.Object3D };
    ...
    private initProjection(minLon: number, maxLon: number, minLat: number, maxLat: number): void {
        ...
        if (bottomLeft === null || topRight === null) {
            throw new Error('Invalid coordinates for projection initialization.');
        }
        ...
        this.projection = (coordinates: [number, number]): [number, number]|null => {
    ...
    private initTrips(): void {
        d3.json(TRIPS_PATH).then((data: unknown) => {
            const datJson = data as { bins: any; bin_size: number };
            this.trips = datJson.bins;
            this.binSize = datJson.bin_size;
        });
    }
    ...
    private drawSwissBorder(): void {
        d3.json(MAP_PATH).then((data: any) => {
            ...
            function getMinMax(data: any, coordinateIndex: number, compareFunc: (a: number, b: number) => boolean, initialValue: number): number {
                return data.features.map((feature: any) => {
                    ...
                    return coordinates.reduce((acc: number, p: number[]) => compareFunc(p[coordinateIndex], acc) ? p[coordinateIndex] : acc, initialValue);
            ...
            const drawFeature = (feature: any) => {
                const coordinates = feature.geometry.coordinates[0];
                const points = coordinates.map((coordinate: number[]) => {
...
private removeHeatMap(): void {
   ...
   * @returns {void}
   */
  public addHeatMap(heatmapMode: string|null): void {
 ...
 */
private getTrainCoordinates(segment: { start_time: number; end_time: number; start_longitude: number; end_longitude: number; start_latitude: number; end_latitude: number }, time: number): number[] {
   ...
   */
  private getTrainColor(segment: { start_time: number; end_time: number; start_longitude: number; end_longitude: number; start_latitude: number; end_latitude: number; start_departure_delay: number; end_arrival_delay: number }, time: number): string {
  ...
  private getTrainAngle(segment: { start_longitude: number; start_latitude: number; end_longitude: number; end_latitude: number }): number {
    if (!this.angleCache) {
      this.angleCache = {};
    }
   ...
   * @param {number[]} trainCoordinates the train coordinates
   * @param {string} trainColor the train color
   * @returns {THREE.Mesh} the train object
   */
  private drawTrain(trainSceneName: string, trainCoordinates: number[], trainColor: string): THREE.Mesh {
